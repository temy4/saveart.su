...
$(document).ready(function() {
        ...
        var calculate = function ( ){
          ...
          if ( $('#service_support').checkbox('isChecked') && price > 0){
            ...
            $('#support_text p').html( 'поддержка:&nbsp;' + prettify(support) + '&nbsp;<span class="rur" style="font-size:20px">p<span style="font-size:20px">уб.</span></span>/месяц, ' );
            if ( free_support > 0 ){
          ...
          $('#total').html( prettify(price) + '&nbsp;<span class="rur">p<span>уб.</span></span>' );
        }
        ...
        function __send_file(){
          ...
          if( file != undefined){
            ...
            $('#wizard-next').text('Загрузка файла...');
            $.ajax({
              url: 'upload',
              dataType: 'json',
              cache: false,
              ...
              error: function(data){
                ...
                switch( data.responseJSON.code ){
                  case -1:
                    msg = 'Слишком большой файл. Попробуйте загрузить файл меньшего размера. Максимальный разрешенный размер файла: <strong>' + numtostr(allown_fsize, ['мегабайт', 'мегабайта', 'мегабайт']) + '</strong>';
                    break;
                  case -2:
                    msg = 'Некорректный формат файла. К загрузке разрешены только файлы типов <strong>DOC / DOCX</strong> (Microsoft Word) или <strong>PDF</strong> (Adobe Acrobat Reader)';
                    break;
                  default:
                    msg = 'Произошла неизвестная ошибка';
                }

                $('#result-failure span').html(msg);
                $('#result-failure').show();
        ...
        function __send_info(){
          $('#wizard-next').text('Отправка заказа...');
